...
const getDashboardData = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    const datasEstudo = [...new Set(estudos.map((e: any) => 
      e.data.toDate().toLocaleDateString('en-CA', { timeZone: 'America/Sao_Paulo' })
    ))].sort().reverse();
...
const updateProfile = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    try {
      ...
      if (curso !== undefined) updates.curso = curso;
      ...
      if (nome !== undefined) userUpdates.name = nome;
      if (curso !== undefined) userUpdates.curso = curso;
      if (faculdade !== undefined) userUpdates.faculdade = faculdade;
