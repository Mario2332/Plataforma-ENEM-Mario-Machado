...
const createAlunoMeta = functions
    ...
    try {
        ...
        const parseDateBrasilia = (dateString) => {
...
const getAlunoResumo = functions
    ...
    try {
        ...
        const datasEstudo = new Set(estudos
            .filter((e) => e.data)
            .map((e) => {
        ...
        const topicosConcluidos = progresso.filter((p) => p.status === "concluido").length;
        // Calcular metas ativas e concluídas
        const metasAtivas = metas.filter((m) => m.status === "ativa" && !m.metaPaiId).length;
        const metasConcluidas = metas.filter((m) => m.status === "concluida" && !m.metaPaiId).length;
        // Calcular média de desempenho nos simulados
        const simuladosComNota = simulados.filter((s) => s.nota !== undefined && s.nota !== null);
        const mediaSimulados = simuladosComNota.length > 0
            ? Math.round(simuladosComNota.reduce((acc, s) => acc + (s.nota || 0), 0) / simuladosComNota.length)
            : 0;
        ...
        let cronogramaDinamicoTipo = null;
        try {
