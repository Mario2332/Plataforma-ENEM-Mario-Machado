...
exports.getTarefasAluno = (0, https_1.onCall)({ region: "southamerica-east1" }, async (request) => {
    ...
    if (filtro === "dia") {
        ...
        query = query
            .where("dataFim", ">=", firestore_1.Timestamp.fromDate(inicioDia))
            .where("dataFim", "<=", firestore_1.Timestamp.fromDate(fimDia));
    }
        ...
        query = query
            .where("dataFim", ">=", firestore_1.Timestamp.fromDate(inicioSemana))
            .where("dataFim", "<=", firestore_1.Timestamp.fromDate(fimSemana));
    }
    else if (filtro === "mes") {
        const inicioMes = new Date(now.getFullYear(), now.getMonth(), 1);
        const fimMes = new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59, 999);
        query = query
            .where("dataFim", ">=", firestore_1.Timestamp.fromDate(inicioMes))
            .where("dataFim", "<=", firestore_1.Timestamp.fromDate(fimMes));
    }
