...
import { DataService, LEGACY_MENTORIA_ID } from "../utils/data-service";
 ...
 */
async function validateMentoria(mentoriaId: string): Promise<FirebaseFirestore.DocumentData> {
  if (mentoriaId === LEGACY_MENTORIA_ID) {
    throw new functions.https.HttpsError(
      "permission-denied",
      "Use as funções do gestor padrão para gerenciar a mentoria legacy"
    );
  }
  ...
  if (mentoriaData.status !== "ativo") {
    throw new functions.https.HttpsError(
      "failed-precondition",
      `Mentoria está ${mentoriaData.status}. Apenas mentorias ativas podem receber novos usuários.`
    );
  }
 ...
 */
async function checkLimites(
  mentoriaId: string,
  mentoriaData: FirebaseFirestore.DocumentData,
  tipo: "alunos" | "mentores"
): Promise<void> {
  ...
  const plano = mentoriaData.plano as MentoriaPlan;
  const limites = mentoriaData.limites || LIMITES_POR_PLANO[plano] || LIMITES_POR_PLANO.basico;
  const maxKey = tipo === "alunos" ? "maxAlunos" : "maxMentores";
  const max = limites[maxKey] || LIMITES_POR_PLANO.basico[maxKey];

  if (ativos >= max) {
    throw new functions.https.HttpsError(
      "resource-exhausted",
      `Limite de ${tipo} atingido (${ativos}/${max}). Faça upgrade do plano para adicionar mais.`
    );
  }
}
...
const createMentorInMentoria = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (!mentoriaId || !email || !password || !nome) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "mentoriaId, email, password e nome são obrigatórios"
      );
    }
...
const getMentoresInMentoria = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (mentoriaId === LEGACY_MENTORIA_ID) {
      ...
      return snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data(), mentoriaId: LEGACY_MENTORIA_ID }));
    }
...
export const mentoriaUsuariosFunctions = {
