...
/**
 * Webhook da Kiwify para cadastro automático de alunos
 * 
 * Endpoint: https://southamerica-east1-plataforma-mentoria-mario.cloudfunctions.net/kiwifyWebhook
 * 
 * Eventos suportados:
 * - order.paid: Quando um pedido é pago (assinatura ativa)
 * - subscription.started: Quando uma assinatura é iniciada
 * 
 * Payload esperado:
 * {
 *   "event": "order.paid",
...
export const kiwifyWebhook = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const webhookData = req.body as KiwifyWebhookPayload;
