...
interface MediasPlataformaParams {
  periodo: "7d" | "30d" | "3m" | "6m" | "1a" | "all";
}

interface MediasPlataformaResult {
  tempoMedio: number; // em minutos
  questoesMedia: number;
  acertosMedia: number;
  taxaAcertoMedia: number; // percentual (média ponderada)
 ...
 */
export const getMediasPlataforma = functions
  .region("southamerica-east1")
  .https.onCall(async (data: MediasPlataformaParams, context): Promise<MediasPlataformaResult> => {
    ...
    // Estrutura para acumular dados de cada aluno
    interface DadosAluno {
      tempoTotal: number;
      questoesTotal: number;
      acertosTotal: number;
      diasEstudo: Set<string>;
    }

    const dadosPorAluno: Map<string, DadosAluno> = new Map();
 ...
 */
export const updatePreferenciaComparacao = functions
  .region("southamerica-east1")
  .https.onCall(async (data: { mostrar: boolean }, context) => {
    const auth = await getAuthContext(context);
 ...
 */
export const getPreferenciaComparacao = functions
  .region("southamerica-east1")
